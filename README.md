...
| `/shipshape`       | Pull the latest `main` on the captain's live repos (dotfiles, vault), re-stow the dotfiles symlinks to point at that main, and update firstmate plus every secondmate in one guarded fast-forward-only pass; skip and report anything dirty, diverged, off-main, or offline |
